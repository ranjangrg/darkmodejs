...
export class DarkModeApp {
	...
	_init_dark_mode_btn() {
		let toggler_elem: HTMLElement = document.getElementById("mode-toggler")!;
	...
	_init_dark_mode_features() {
		const mode_btn = document.getElementById("mode-btn")! as HTMLInputElement;
	
		mode_btn.onchange = (e) => {
			let curr_mode: string = "dark-mode";
			if (mode_btn.checked) {
				curr_mode = "light-mode";
			} 
			let all_elems = document.getElementsByClassName(curr_mode) as HTMLCollection;
	...
	_init_color_mode_within_dom() {
		...
		let node_list: NodeListOf<HTMLElement> = document.querySelectorAll("[data-dm-enable-color-mode] [data-dm-use-color-mode]");
